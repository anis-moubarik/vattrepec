...
if ($ARGV[0] eq '') {
    require("dspace2redif_1.conf") or die "Could not find configuration file dspace2redif.conf";
} else {
...
while($chunkflag==0){
    ...
    my $str=$ua->get($URL)->decoded_content;
	...
	if ($str =~ /status="deleted"/) {
	    ...
	    #use the dc.relation.numberinseries for Number field
	    my ($numofseries) = ($str =~ /<dc:relation>([a-zA-z]*[0-9]*:*[a-zA-z]*[0-9]*)<\/dc/g);
		...
		print OUTA "Number: $numofseries \n";
		print OUTA "Creation-Date: $pubyr \n";
		print OUTA "File-URL: $url \n";
		print OUTA "Handle: RePEc:$Elements[1]:$Elements[2]:$numofseries \n";
		$nrtem++;
	    } else {
